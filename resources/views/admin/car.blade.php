...
<body class="bg">
    ...
    <h1 class="fw-bold text-center mt-5 pt-5  text-white">entered cars</h1>

    <div class="search-container">
        <!-- Search form -->
        <form action="{{ route('submission.search_car') }}" method="GET">
            <input type="text" name="query" placeholder="Search by ID or Name">
            <button type="submit">Search</button>
        </form>
        <!-- Back button -->
        <button type="button" onclick="window.location.href='{{ route('submission.search_car') }}'">Back</button>
    </div>
    ...
    <table class="w-75 mx-auto" border="5">
        ...
        <tbody class="bg-2">
            ...
            <tr>
                ...
                <td>
                    <!-- Delete form -->
                    <form action="{{ route('submission.destroy_car', $submission->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
